  ...
  return (
    <>
      ...
      {(['DEFINE', 'DISCOVER', 'VALIDATE', 'DESIGN', 'BUILD', 'LAUNCH'] as const).map((phase) => (
        <section key={phase}>
          <h2 className="mb-3 text-xs font-bold tracking-[.2em] text-accent">{phase}</h2>
            ...
            {previewMissions
              ...
              .map((m) =>
                m.status === 'locked' ? (
                  ...
                  >
                    <span className="font-mono text-lg font-bold">{m.day}</span>
                  ...
                  <Link
                    ...
                    className={`rounded-2xl border p-3 text-center transition ${
                      m.status === 'available'
                        ? 'border-gold bg-accent text-white shadow-[0_0_0_2px_rgba(255,194,77,0.5)]'
