...
2026-09-21. Owner: PO 정아의 `codex/mission-proof` 작업. 사람 Reviewer는 PR에서 팀원 지정 필요(승인 미수령). 선행 PR #8 `codex/mission-camera`; main의 팀원 통합 PR #9는 반영된 상태다. 기존 Google 인증/참가 규칙·네비 디자인은 재설계하지 않았다.
...
1. PR #8 사람 리뷰·필수 CI 후 병합. 이 PR은 #8 위에 쌓은 의존 PR이므로 #8 완료 후 base를 main으로 바꾸고 최종 diff/CI를 재확인한다. 강제 push하지 않는다.
