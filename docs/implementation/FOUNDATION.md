...
- 사람 리뷰는 요청했으며 `leejidev`의 1차 변경 요청과 후속 최종 검토 의견을 반영했다. 2026-09-18 팀 회의 확인과 PO 결정에 따라 Foundation PR #2는 [병합 Gate 예외](../decisions/2026-09-18-foundation-merge-gate-exception.md)의 조건을 충족하면 별도 GitHub `Approve` 없이 병합할 수 있다.

## 팀 기능 PR 통합 기준

- FOUNDATION PR #2가 병합되기 전 기능 PR은 `codex/app-foundation`을 임시 base로 둘 수 있지만, FOUNDATION의 확정 계약을 바꾸어서는 안 된다. PR #2 병합 뒤 최신 `main`을 기준으로 재정렬하고 CI를 다시 통과해야 한다.
