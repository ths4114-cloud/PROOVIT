...
Mac에서는 Finder에서 `PROOVIT 화면보기.command`를 더블클릭하면 됩니다. 의존성이 없으면 자동 설치하고, 로컬 테스트 DB와 앱을 실행한 뒤 `/preview`를 열어 홈까지 자동 진입합니다. 실제 Supabase 계정·SMTP는 사용하지 않습니다. 종료는 열린 터미널에서 Enter입니다.
...
- `/login`: 이메일 OTP 가입·로그인, 재발송·잘못된 코드·만료 오류 안내.
...
`.env.local`의 다음 세 항목을 채우세요.

| 변수 | 값 |
| --- | --- |
| `NEXT_PUBLIC_SUPABASE_URL` | Supabase Project URL |
| `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` | Supabase publishable key. 로컬 CLI가 anon key만 제공하면 호환 사용 가능 |
...
4. Auth의 Email 로그인/가입을 활성화하고 익명 로그인은 끕니다.
5. Auth 이메일 템플릿의 **Magic Link와 Confirm Signup 모두** `supabase/templates/otp.html` 내용으로 설정합니다. `{{ .Token }}`이 포함되어야 화면에 입력할 인증 코드가 전달됩니다. OTP 길이는 6자리(앱은 6~8자리 지원), 만료 시간은 서비스 정책에 맞게 설정하세요.
6. 발송 가능한 SMTP를 연결하고 테스트 이메일이 도착하는지 확인합니다. 기본 이메일 서비스의 발송 대상·빈도 제한을 확인하세요. 대중 공개 전 발송 제한과 남용 방지 설정을 점검합니다.
7. Site URL은 로컬 개발에서 `http://localhost:3000`, 배포 후에는 실제 HTTPS 주소로 설정합니다. 이번 방식은 링크 callback 대신 코드 입력으로 인증하므로 `/auth/callback`은 필요하지 않습니다.
8. Data API에서 `public` schema와 RPC 접근이 활성화되어 있는지 확인합니다. `private` schema는 노출하지 않습니다. 테이블 권한과 RLS는 migration에 명시되어 있습니다.
...
로컬 메일함 주소는 `supabase status` 출력에서 확인합니다. 템플릿과 OTP 6자리 설정은 `supabase/config.toml`에 포함되어 있습니다. CLI의 사용자 홈 쓰기가 제한되는 환경에서는 `SUPABASE_HOME="$PWD/work/supabase-cli"`를 명령 앞에 붙일 수 있습니다.

## 실제 데이터로 확인할 순서

1. 챌린지에서 규칙 확인 후 참가 → 이메일 입력 → 이메일 인증 코드 입력.
...
7. 실제 iPhone Safari / Android Chrome에서 설치, 재실행, 화면 회전, 이메일 앱 왕복, 세션 갱신을 확인하세요.
...
- E2E: 앱은 실제 프로덕션 서버로 실행. 이메일 인증·HTTP Data API만 테스트 대역이며 실제 migration을 PostgreSQL 엔진에서 실행합니다. 외부 메일을 보내지 않습니다. 고정 코드 `123456`은 **E2E 대역에서만** 유효합니다.
...
2. `feat/participant-challenge-auth-home` Pull Request에서 사람 리뷰와 필수 CI를 받습니다.
3. Vercel에 Next.js 프로젝트로 연결하고 **Preview 전용 Supabase 프로젝트**의 환경변수 세 개를 설정합니다. Build Command는 `npm run build`입니다.
4. 해당 개발/스테이징 DB에 migration, 승인된 챌린지·미션 데이터를 준비하고 Auth 템플릿·SMTP를 설정합니다.
5. Preview HTTPS 주소에서 실제 이메일 로그인·참가·조회·로그아웃·PWA 설치 검증을 수행합니다.
...
이번 변경은 `codex/app-foundation`을 base로 한 `feat/participant-challenge-auth-home` 브랜치에 모았습니다. 범위는 참가자용 챌린지 소개, 이메일 OTP 로그인, 중복 없는 참가, 홈 데이터, PWA와 해당 DB·브라우저 테스트입니다. 기존 미션 상세·촬영·결과 미리보기는 그대로 유지합니다. Feature Owner와 Reviewer는 팀에서 지정합니다.
...
- 실패 시 서버 로그의 `operation`, 임의 `reference`, 일반 오류 코드를 확인합니다. 이메일·토큰·키는 로그에 기록하지 않습니다.
- DB 실패를 0점/참가 성공으로 바꾸지 않고 오류와 재시도를 표시합니다.
- 앱 문제는 이전 Vercel 배포로 복귀할 수 있습니다. DB와 이메일 발송 이력은 앱 rollback으로 되돌아가지 않습니다.
...
실제 Supabase Auth/PostgREST/SMTP, 장기 세션 만료·갱신, Vercel Preview, iOS/Android 실기기 설치, Supabase advisors, 팀 PO 정책 승인과 사람 리뷰는 미실행입니다. 소스 구현과 로컬 검증이 끝나도 저장소 Done 기준의 병합 가능/출시 완료는 아닙니다.
